...
export function MoodChart({ entries }: MoodChartProps) {
  ...
  const getChartData = () => {
    ...
    const groupedData: { [key: string]: { mood: number[], count: number } } = {};
    ...
    filteredEntries.forEach(entry => {
      ...
      if (!groupedData[key]) {
        groupedData[key] = { mood: [], count: 0 };
      }
      
      if (entry.moodScore) groupedData[key].mood.push(entry.moodScore);
    ...
    const chartData = Object.entries(groupedData).map(([date, data]) => ({
      ...
      mood: data.mood.length > 0 ? 
        Number((data.mood.reduce((a, b) => a + b, 0) / data.mood.length).toFixed(1)) : 0,
      count: data.count
    }));
  ...
  const avgMood = chartData.length > 0 
    ? (chartData.reduce((sum, day) => sum + (day.mood || 0), 0) / chartData.length).toFixed(1)
    : "0.0";
      ...
      <CardContent>
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 text-center">
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-medium">Avg Mood</p>
            <p className="text-lg font-semibold text-foreground">{avgMood}</p>
          </div>
          <div className="space-y-1">
